...
- **Nav toggle / theme toggle script is currently duplicated inline in every single page** (`index.html`, both list pages, all detail pages — 6+ copies of the same ~40 lines). This is known tech debt, not an intentional pattern to replicate when adding new pages.
...
### Known quirks
- `README.md` is currently committed with UTF-16LE encoding (with BOM) rather than plain UTF-8 — `git diff` on it shows as binary. Worth fixing but not blocking; don't be surprised by it.
- `AGENTS.md` in this repo is stale (describes an earlier "theme exploration, placeholder-only" phase that no longer reflects the current, fully-built-out site) — prefer this file over it.
