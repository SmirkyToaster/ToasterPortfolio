...
`js/link-helpers.js` exposes `window.RouteLinks.detailRoute(basePath, slug)` / `sectionRoute(basePath)` so card-grid rendering code builds consistent relative URLs instead of hand-concatenating paths.
